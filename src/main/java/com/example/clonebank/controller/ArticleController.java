package com.example.clonebank.controller;

import com.example.clonebank.dto.ArticleForm;
import com.example.clonebank.entity.Article;
import com.example.clonebank.repository.ArticleRepository;
...
public class ArticleController {
    ...
    public String show(@PathVariable("id") Long id, Model model){
        log.info("id = " + id);
        // 1. id로 데이터를 가져옴
        // 밑에 방법은 자바 8에서 사용하는 표현이므로 이해하면 좋음.
//        Optional<Article> articleEntity = articleRepository.findById(id);
    ...
    public String index(Model model){
        // 1. 모든 article을 가져온다.
        // findAll은 반환이 Iterable 객체다. 그러므로 처리하려면 캐스팅하거나, iterable로 바꿔야함.
